...
(function() {
    ...
    var DMS_FORMAT_REGEX = /^[NSEW]?\s*(\d{1,3})°?(?:(\d{1,2})'?(?:(\d{1,2}(?:.\d+)?)"?)?)?\s*[NSEW]?$/
    ...
    function magellan() {
        ...
        magellan.toDD = function() {
            ...
            var formatted;
            if (coordinate.degrees > 0) formatted = (coordinate.degrees + decimal).toString();
            else formatted = (coordinate.degrees - decimal).toString()
        ...
        magellan.toDMS = function() {
            return new String(Math.abs(coordinate.degrees) + '°'
                + coordinate.minutes + '\''
                + coordinate.seconds.toFixed(4) + '" '
                + (coordinate.direction ? coordinate.direction : '')).trim();
        }

        // Treat the current coordinate as latitude
        magellan.latitude = function() {
            ...
            if (!coordinate || Math.abs(coordinate.degrees) > 180
                    ...
                    // and the direction is North or South
                    || coordinate.direction == EAST || coordinate.direction == WEST) {

                    // Something in the above validation logic failed, so return null
                    return null;
            }
        ...
        // Treat the current coordinate as longitude
        magellan.longitude = function() {
            ...
            if (!coordinate || Math.abs(coordinate.degrees) > 180
                    ...
                    // and the direction is East or West
                    || coordinate.direction == NORTH || coordinate.direction == SOUTH) {

                    // Something in the above validation logic failed, so return null
                    return null;
            }

            // A-OK, so return magellan for chaining
            return magellan;
        }
